...
test "Allocates and frees" {
    var simple = SimpleAllocator.init();
    defer simple.deinit();

    const allocator = simple.allocator();
    const a = try allocator.create(u32);
    const b = try allocator.create(u32);
    a.* = 10;
    b.* = 20;
...
test "Reuses freed block" {
    ...
    try std.testing.expectEqual(first_addr, @intFromPtr(second.ptr));
}
